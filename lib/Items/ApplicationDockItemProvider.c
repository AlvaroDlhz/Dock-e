...
static gboolean ___lambda100_ (Block10Data* _data10_);
...
static gboolean
___lambda100_ (Block10Data* _data10_)
{
	PlankApplicationDockItemProvider* self;
	gboolean result;
	self = _data10_->self;
                                                                 ...
                                                                 gboolean is_retry)
{
	...
	if (!is_retry) {
		g_idle_add_full (G_PRIORITY_DEFAULT_IDLE, ____lambda100__gsource_func, block10_data_ref (_data10_), block10_data_unref);
		_g_object_unref0 (alternate_item);
		_g_object_unref0 (current_item);
		_g_variant_iter_free0 (prop_iter);
		_g_free0 (app_uri);
