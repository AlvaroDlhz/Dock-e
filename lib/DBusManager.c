...
VALA_EXTERN PlankDBusItems* plank_dbus_items_construct (GType object_type,
                                            PlankDockController* _controller);
static gboolean __lambda160_ (PlankDBusItems* self);
...
static gboolean
__lambda160_ (PlankDBusItems* self)
{
	gboolean result;
...
{
	gboolean result;
	result = __lambda160_ ((PlankDBusItems*) self);
	return result;
}
...
plank_dbus_items_handle_elements_changed (PlankDBusItems* self)
{
	...
	self->priv->changed_timer_id = g_timeout_add_full (G_PRIORITY_DEFAULT, (guint) 500, ___lambda160__gsource_func, g_object_ref (self), g_object_unref);
}
